@php
    ...
    $selectedType = $question?->question_type?->value ?? '';
...
<div class="question-block border rounded p-4 mb-4 bg-white" data-testid="question-block">
    ...
    <div class="mb-2">
        ...
        <input type="text" name="questions[{{ $qIndex }}][body]" value="{{ $bodyValue }}" data-testid="question-body-input" class="border rounded w-full px-2 py-1">
    </div>
    ...
    <div data-testid="scale-section" class="mb-2" @if (! $showScale) hidden @endif>
        <div class="mb-1">
            ...
            <input type="text" name="questions[{{ $qIndex }}][scale_min_label]" value="{{ $scaleMinLabel }}" data-testid="scale-min-label-input" class="border rounded w-full px-2 py-1">
        </div>
        <div>
            <label class="block text-sm mb-1">5側ラベル（任意）</label>
            <input type="text" name="questions[{{ $qIndex }}][scale_max_label]" value="{{ $scaleMaxLabel }}" data-testid="scale-max-label-input" class="border rounded w-full px-2 py-1">
        </div>
    </div>
</div>
