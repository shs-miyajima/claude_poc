...
| 項目 | パス / 値 |
|------|-----------|
| PHPUnit 実行ログ | `docker compose exec app php artisan test`（全体 226 件成功、うち本機能 88 件） |
...
- 上記②の対応中に、設問・選択肢のバリデーションエラー発生時、`_form.blade.php` が常に DB の
  `$survey->questions`（作成時は空）のみを描画しており、`old('questions')` を参照していなかったため、
  再表示時に入力済みの設問・選択肢がフォームごと消えてしまう不具合を検出した。`_form.blade.php` で
  `old('questions')` を優先して復元し、`_question.blade.php`/`_choice.blade.php` を Eloquent モデルと
  `old()` 配列の両方を扱えるよう連想配列ベースに統一して解消した。回帰確認として PHPUnit に
  `test_create_validation_failure_preserves_question_input_and_shows_field_errors` /
  `test_update_validation_failure_preserves_question_input_and_shows_field_errors` の 2 件を追加
  （正式な Test ID は付与せず、計画外の追加回帰テストとして実装）
- 上記以外の残課題なし
