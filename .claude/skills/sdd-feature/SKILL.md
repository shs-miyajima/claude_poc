...
- [ ] §6 影響範囲に、既存の画面・API・共通部品・テーブルへの波及を `IMPACT-xx` 付きで洗い出す
      （影響なしの判断も理由を明記する）
- [ ] **承認確認を提示する前に**、独立レビュー用サブエージェント（`Agent` ツール・
      `subagent_type: sdd-design-reviewer`、読み取り専用）を起動し、
      `02-design-review-checklist.md` の全項目を検証させる（IMPACT はコードベース検索で裏取り）
...
- [ ] テスト種別ごとに CSV を分けてケース一覧を記載:
  - Playwright E2E: `03-test-plan.csv`（作成時は `testing-playwright` skill を参照）
  - PHPUnit: `03-test-plan-phpunit.csv`（Service 等の単体テストがある場合。`laravel-conventions` skill を参照）
  - Vitest: `03-test-plan-vitest.csv`（JavaScript 単体テストがある場合。`testing-vitest` skill を参照）
...
- [ ] 独立したレビュー用のサブエージェント（`Agent` ツール・`subagent_type: sdd-plan-reviewer`、
      新規セッション・読み取り専用。定義: `.claude/agents/sdd-plan-reviewer.md`）を起動し、
...
- [ ] PHPUnit（Service 単体、該当時。`laravel-conventions` skill を参照）
...
- **承認直後に git commit する**（`[SDD][<slug>] フェーズN(<フェーズ名>) approved`）。
  フェーズ 4 は完了報告作成後にコミット（`[SDD][<slug>] フェーズ4(実装・テスト) 完了`）。
...
| 種別 | リソース | パス |
|---|---------|------|
| Rule（常時適用） | ワークフロー規約 | `.claude/rules/sdd-workflow.md` |
| Skill（必要時に参照） | Laravel 規約 | `.claude/skills/laravel-conventions/SKILL.md` |
| Skill（必要時に参照） | フロント規約 | `.claude/skills/frontend-vite-tailwind/SKILL.md` |
| Skill（必要時に参照） | Playwright 規約 | `.claude/skills/testing-playwright/SKILL.md` |
| Skill（必要時に参照） | Vitest 規約 | `.claude/skills/testing-vitest/SKILL.md` |
